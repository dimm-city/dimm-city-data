...
strapi_controller.verifyToken = async function verifyToken(ctx) {
  ...
  try {
    ...
    const secondaryUser = await strapi.query('user', 'users-permissions').findOne({ id: decoded.id });

    //Get the profile for the secondary user
    const secondaryUserProfile = await strapi.query('profile').findOne({ user: secondaryUser.id });

    //Get the profile for the logged in user
    const primaryUserProfile = await strapi.query('profile').findOne({ user: ctx.state.user.id });
    ...
    await strapi.query('profile').update({ id: primaryUserProfile.id }, primaryUserProfile);
    ...
    ctx.send({ message: 'Secondary user added to primary user', profile: primaryUserProfile });
