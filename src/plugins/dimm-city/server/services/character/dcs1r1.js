...
//   //   const contractService = strapi.service("plugin::dimm-city.contracts");
...
module.exports = {
  async initializeEntity(token) {
    const race = await strapi.entityService.findMany("plugin::dimm-city.race", {
      filters: {
        name: "Rabbit",
      },
    });
  ...
  async extendTokenMetadata(token, character) {
    if (!character) {
      let characters = await strapi
        .entityService("plugin::dimm-city.character")
        .find({
          filters: {
            token: {
              id: token.id,
